...
# -*- coding: utf-8 -*-
"""
Cron任务管理模块（修复subprocess参数冲突 + 改名set-cron）
"""
import os
import sys
import subprocess
...
# ========== Cron核心功能（修复subprocess参数冲突） ==========
def set_cron():
    """配置定时任务：修复subprocess参数冲突 + 兼容空crontab"""
    print_color("===== 开始配置定时任务 =====", "YELLOW")
    if not CRON_TASKS:
        print_color("❌ CRON_TASKS配置为空，配置失败", "RED")
        sys.exit(1)
    
    # 1. 备份原有crontab（兼容空crontab，避免subprocess参数冲突）
    ...
    try:
        # 修复：移除capture_output，直接重定向stdout到文件，stderr忽略
        subprocess.run(
            ["crontab", "-l"],
            stdout=open(backup_file, "w", encoding="utf-8"),
            stderr=subprocess.DEVNULL,  # 忽略no crontab for root的错误
            check=False
        )
    ...
    # 2. 读取原有crontab（修复：用stdout=PIPE替代capture_output，避免参数冲突）
    result = subprocess.run(
        ["crontab", "-l"],
        stdout=subprocess.PIPE,       # 替代capture_output=True，单独捕获stdout
    ...
    # 5. 导入新crontab（修复：用stdout/stderr PIPE替代capture_output）
    try:
        ...
        cron_lines = [line for line in show_result.stdout.splitlines() if CRON_TASK_MARK in line]
        ...
        if os.path.exists(backup_file) and os.path.getsize(backup_file) > 0:
            subprocess.run(["crontab", str(backup_file)], stderr=subprocess.DEVNULL, check=False)
...
def check_cron():
    """检查cron配置（修复subprocess参数冲突）"""
    print_color("===== 当前本项目Cron配置 =====", "YELLOW")
    result = subprocess.run(
        ["crontab", "-l"],
        stdout=subprocess.PIPE,       # 替代capture_output=True
        stderr=subprocess.DEVNULL,
        text=True,
        check=False
    )
...
def cancel_cron():
    ...
    print(remaining or "无")
    
    if TEMP_CRON_FILE.exists():
        os.remove(TEMP_CRON_FILE)

def clear_all_cron():
    """高危：删除所有Cron任务（修复subprocess参数冲突）"""
