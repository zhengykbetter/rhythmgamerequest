...
def parse_intent(query: str) -> dict:
    safe_print(f"   📥 [intent_parser] 输入: {query}")
    
    if not query:
        safe_print(f"   ⚠️  [intent_parser] 输入为空")
    ...
    try:
        ...
        safe_print(f"   🔤 [intent_parser] 调用 LLM (意图解析)...")
        safe_print(f"   📝 [intent_parser] LLM 输入长度: {len(prompt)} 字符")
        ...
        safe_print(f"   ✅ [intent_parser] LLM 调用成功")
        safe_print(f"   🔢 [intent_parser] Token 消耗: {token_used}")
        safe_print(f"   📤 [intent_parser] LLM 输出: {raw_result[:100]}...")
        ...
        safe_print(f"   ❌ [intent_parser] 出错: {str(e)}")
